...
int decryptData(char *data, int dataLength)
{
	...
	__asm {
		...
		mov eax, gNumRounds// Push total number of round to eax
		push eax // Top of stack is the round number

	ROUND :
		pop ecx // Pop the reverse round number into ecx
		sub ecx, 0x01 // subtract one to round number
	...
	DEC_EXIT :
		...
		cmp[esp], 0x00 // cmp round number to number of rounds
		jg ROUND // jump to ROUND if less than
		pop eax // clear stack
	}
	return resulti;
} // decryptData
